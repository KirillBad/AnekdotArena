...
async def show_admin_panel(callback: CallbackQuery):
    await callback.message.edit_text(
        text="🚔 Админ панель 🚔", reply_markup=admin_panel_kb()
    )
...
async def show_admin_panel(
    ...
    anecdotes = await AnecdoteDAO.find_all(
        session_without_commit, order_by="report_count"
    )
    if not anecdotes:
        await callback.message.edit_text(
            "Нет анекдотов с репортами", reply_markup=back_to_admin_panel_kb()
        )
        return

    await state.update_data(
        anecdote_ids=[anecdote.id for anecdote in anecdotes], current_index=0
    )
...
async def delete_anecdote(
    ...
    await AnecdoteDAO.delete(
        session_with_commit, AnecdoteFilter(id=anecdote_ids[current_index])
    )
...
async def cancel_reports(
    ...
    await AnecdoteDAO.update(
        session_with_commit,
        AnecdoteUpdate(report_count=0),
        AnecdoteFilter(id=anecdote_ids[current_index]),
    )
